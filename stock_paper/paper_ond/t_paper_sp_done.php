...
<?php
  try{
    ...
    if($paper_done_type=='t_paper_sp_add'){
      $sql='INSERT INTO mst_paper_ond(p_color,p_atsusa,p_size,p_name,p_me,p_number,p_entryuser) VALUES (?,?,?,?,?,?,?)';
      ...
      $sql='UPDATE mst_paper_ond SET p_color=?,p_atsusa=?,p_size=?,p_name=?,p_me=?,p_number=?,p_entryuser=? WHERE p_code=?';
      $stmt=$dbh->prepare($sql);
      $data[]=$paper_color;
      $data[]=$paper_atsusa;
      $data[]=$paper_size;
...
<br>
<p><a href="t_paper_list_ond.php#paper_sp" class="btn-simple">戻る</a></p>
