...
#    print '<form method="post" action="../staff_login/staff_login.html"><input type="submit" value="ログイン" class="btn-shadow"></form>';
    ...
    print '　<p><a href="../paper/t_paper_list.php" class="btn-simple">用紙管理【色上質】</a>';
    print '　<a href="../paper/t_paper_list_r.php" class="btn-simple">用紙管理【レザック】</a>';
...
<h1 class="ond f_clear">用紙管理【色上質（オンデマンド）】</h1>
...
<div id=paper_sp>
  ...
  //      $sql='SELECT code,paper_name,paper_size,paper_me,paper_atsusa,paper_price,entry FROM t_paper_sp WHERE 1'; //登録code順
          $sql='SELECT p_code,p_color,p_atsusa,p_size,p_name,p_me,p_number,p_entryuser,p_entry FROM mst_paper_ond WHERE p_name="日本の色上質" OR p_name="紀州の色上質" OR p_name="その他" ORDER BY p_color,p_name,p_atsusa,p_size,p_me ASC'; //用紙名昇順
          $stmt=$dbh->prepare($sql);
          $stmt->execute();
